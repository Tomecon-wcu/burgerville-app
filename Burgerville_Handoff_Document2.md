...
- Students sign in with just an email (a one-time code, no password), submit predictions, and
  can revise their submission any time before the deadline.
- Season 1 has been run and scored; Results screen shows individual breakdown and team
  standings correctly.
...
Browser (Burgerville.html, hosted on GitHub Pages)
   ...
   │
   └─ Three Edge Functions (Deno/TypeScript, deployed to Supabase)
...
| What | Where |
|---|---|
| Client (the whole app) | `Burgerville.html` — one file, uploaded to GitHub Pages |
